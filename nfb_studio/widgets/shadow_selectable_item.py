...
class ShadowSelectableItem(QGraphicsItem):
    ...
    Shadow selection occurs independently from normal selection. It is not enabled by rubber-band selection. Shadow
    selection is meant for items that derive their selection status from other factors. This class is intended for
    inheritance. When shadow selected, it will send an itemChange event with `change =
    ShadowSelectable.ItemShadowSelectedChange`, as well as `ShadowSelectable.ItemShadowSelectedHasChanged`.
    """
